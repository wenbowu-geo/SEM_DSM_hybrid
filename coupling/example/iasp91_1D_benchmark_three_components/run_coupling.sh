...
dir_DSM=../../../DSM/example/iasp91_1D_benchmark_three_components
...
#compute radial component? If .true., make sure you have computed the Green's functions associated with radial single force f_theta and f_phi. 
radial_component=.true.
#compute tranverse component? If .true., make sure you have computed the Green's functions associated with tranverse single force f_phi and t_theta. 
transverse_component=.true.
...
#radial component
mkdir -p DSM_input/rcomp
mkdir -p DSM_input/rcomp/disp_solid
mkdir -p DSM_input/rcomp/sigma

#tranverse component
mkdir -p DSM_input/tcomp
mkdir -p DSM_input/tcomp/disp_solid
mkdir -p DSM_input/tcomp/sigma
...
#copy DSM outputs
  ...
  echo "Large size of data, it takes time..."
  cp  ${dir_DSM}/fr/OUTPUT_GREENS/sigma/freq* DSM_input/zcomp/sigma
  cp  ${dir_DSM}/fr/OUTPUT_GREENS/disp_solid/freq* DSM_input/zcomp/disp_solid 
  cp ${dir_DSM}/fr/OUTPUT_GREENS/Green_Par_forConvolution DSM_input/Green_Par
  echo
  echo "copy the DSM Green's Function into DSM_input/rcomp/"
  echo "Large size of data, it takes time..."
  cp  ${dir_DSM}/ftheta/OUTPUT_GREENS/sigma/freq* DSM_input/rcomp/sigma
  cp  ${dir_DSM}/ftheta/OUTPUT_GREENS/disp_solid/freq* DSM_input/rcomp/disp_solid 
  echo
  echo "copy the DSM Green's Function into DSM_input/tcomp/"
  echo "Large size of data, it takes time..."
  cp  ${dir_DSM}/fphi/OUTPUT_GREENS/sigma/freq* DSM_input/tcomp/sigma
  cp  ${dir_DSM}/fphi/OUTPUT_GREENS/disp_solid/freq* DSM_input/tcomp/disp_solid 
  echo
...
echo "Preparing the PBS script for job submision"
                ...
                echo "#PBS -N 1D_iasp91_3comp_coupling" >>submit_coupling.pbs
