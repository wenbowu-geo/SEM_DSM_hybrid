        !COMPILER-GENERATED INTERFACE MODULE: Fri Aug  3 10:43:36 2018
        ! This source file is for reference only and may not completely
        ! represent the generated interface used by the compiler.
        MODULE COMPUTE_ADD_SOURCES_ACOUSTIC_BACKWARD__genmod
          INTERFACE 
            SUBROUTINE COMPUTE_ADD_SOURCES_ACOUSTIC_BACKWARD(NSPEC_AB,  &
     &IBOOL,NSOURCES,MYRANK,IT,ISLICE_SELECTED_SOURCE,                  &
     &ISPEC_SELECTED_SOURCE,SOURCEARRAYS,KAPPASTORE,ISPEC_IS_ACOUSTIC,  &
     &SIMULATION_TYPE,NSTEP,NGLOB_ADJOINT,B_POTENTIAL_DOT_DOT_ACOUSTIC)
              INTEGER(KIND=4) :: NGLOB_ADJOINT
              INTEGER(KIND=4) :: NSOURCES
              INTEGER(KIND=4) :: NSPEC_AB
              INTEGER(KIND=4) :: IBOOL(5,5,5,NSPEC_AB)
              INTEGER(KIND=4) :: MYRANK
              INTEGER(KIND=4) :: IT
              INTEGER(KIND=4) :: ISLICE_SELECTED_SOURCE(NSOURCES)
              INTEGER(KIND=4) :: ISPEC_SELECTED_SOURCE(NSOURCES)
              REAL(KIND=8) :: SOURCEARRAYS(NSOURCES,3,5,5,5)
              REAL(KIND=8) :: KAPPASTORE(5,5,5,NSPEC_AB)
              LOGICAL(KIND=4) :: ISPEC_IS_ACOUSTIC(NSPEC_AB)
              INTEGER(KIND=4) :: SIMULATION_TYPE
              INTEGER(KIND=4) :: NSTEP
              REAL(KIND=8) :: B_POTENTIAL_DOT_DOT_ACOUSTIC(NGLOB_ADJOINT&
     &)
            END SUBROUTINE COMPUTE_ADD_SOURCES_ACOUSTIC_BACKWARD
          END INTERFACE 
        END MODULE COMPUTE_ADD_SOURCES_ACOUSTIC_BACKWARD__genmod
