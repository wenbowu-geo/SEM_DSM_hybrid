...
!     print *,'iface',iface,ispec_is_elastic(ispec)
  ...
  print *,'num_faces_this_mediatype',num_faces_this_mediatype
  ...
  print *,'num_faces_this_mediatype',num_faces_this_mediatype,media_type
  iface_temp=0
  do iface=1,num_abs_boundary_faces
     ispec = abs_boundary_ispec(iface)
     if(ispec_is_acoustic(ispec).and.media_type.eq.2) then
        ...
        print *,'acoustic ispec',iface
     else if(ispec_is_elastic(ispec).and.media_type.eq.1) then
        iface_temp=iface_temp+1
        iface_this_mediatype(iface_temp)=iface
     else if(.not.ispec_is_acoustic(ispec).and..not.ispec_is_elastic(ispec)) then
       print *,ispec_is_acoustic(ispec),ispec_is_elastic(ispec),media_type,&
  ...
  print *,'depth_list begin',myrank,npoint_bound
...
!  print *,'allocate finished',myrank
...
if(ndepth_table.gt.0) then
 ...
 if(myrank.eq.0) &
  ...
  if(npoint_bound.gt.0.and.ipoint_temp.ne.npoint_bound) &
        print *,myrank,"Error counting",ipoint_temp,npoint_bound
 ...
 if(myrank.eq.0) then
  ...
  do idepth=1,ndepth_table
    ...
    do izone=1,n_structure_zone
      ...
      if(idepth.eq.25.and.izone.eq.10) print *,'discon detect',&
...
!receive the arrays
 ...
 do idepth=1,ndepth_table 
    print *,'construct distance table',idepth
    idist=0
    do ipoint=1,npoint_bound
      if(depth_id(ipoint).eq.idepth) then
          idist=idist+1
...
!          if(dist_id(ipoint).eq.0) print
!          *,"Error",dist_bound(ipoint),idist,ndist(idepth)
      end if
    end do
    if(idist.ne.ndist(idepth)) print *,myrank,"Error",idist,ndist(idepth)
...
!      write(113,*) dist_table_global(idist)/r*(180.d0/PI)
 ...
 if(myrank.eq.0) close(113)
  ...
  print *,'move_idepth',move_idepth(:),idepth_is_discont(:)
  ipoint=0
  do iface=1,num_faces_this_mediatype
    do igll=1,NGLLSQUARE
     ipoint=ipoint+1
  ...
  print *,'num_faces',myrank,num_faces_this_mediatype,media_type
  ipoint=0
  do iface=1,num_faces_this_mediatype
    do igll=1,NGLLSQUARE
     ipoint=ipoint+1
...
!      print *,'test',myrank,ipoint,idepth_test,ndepth_table,&
...
!      print *,'id_inlocal',id_inlocal(ipoint),depth_bound(ipoint),myrank,ipoint
  end do
  print *,'ndepth_table1',n_table,table_local(:),myrank
...
!         print *,'recv0'
    ...
    do iproc=1,NPROC-1
!         print *,'recv ndepth',iproc
         call recv_i(ntable_eachproc(iproc+1),1,iproc,0)
!         print *,'recv ndepth1',iproc,ndepth_eachproc(iproc+1)
...
!            print *,'n_table',n_table,max_n_table
...
!merge the local depth tables, and then construct map_localtoglobal
  if(myrank.eq.0) then
!    print *,'depth_all',table_all_local(1:ntable_all_local)
    do itable=1,ntable_all_local
      if(abs(table_all_local(itable)).lt.tolerence_value) then
!       print *,'depth0',table_all_local(itable)
!WENBO
      ...
      if(itable_test.eq.1) then
        if(abs(table_global(itable_test)-table_all_local(itable)).lt. &
         ...
         if(n_table_global.gt.max_ntable_global) &
          ...
          print *,'case3',table_global(itable_test),table_all_local(itable),&
...
!        print *,'map',map_all(:)
        ...
        if(abs(table_global(itable_test)-table_all_local(itable)).lt. &
               tolerence_value) then
         map_all(itable)=itable_test
         print *,'case1',table_global(itable_test),table_all_local(itable)
         ...
         print *,'case2',table_global(itable_test),table_all_local(itable)
        else
         do itable_temp=n_table_global,itable_test,-1
          if(itable_temp.lt.-1) stop 'Error itable_temp' 
          table_global(itable_temp+1)=table_global(itable_temp)
...
!      print *,'map_all',idep,map_all(idep),idepth_test,ndepth_table_global,&
...
!    print *,'table',map_all(:)
    ...
    if(n_table.gt.0) then
!        if(myrank.eq.61) print *,'ndepth_table',ndepth_table,map_localtoglobal
        req_recv=myrank
