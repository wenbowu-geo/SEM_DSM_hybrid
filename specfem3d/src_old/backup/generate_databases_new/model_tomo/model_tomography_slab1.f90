...
!in unit of degree, roughly 112km at free surface and smaller for deeper earth
double precision, parameter::degtorad=PI/180.0
double precision, parameter::xi0_taperleft_deg=0.3*degtorad 
double precision, parameter::xi1_taperleft_deg=0.7*degtorad
double precision, parameter::xi0_taperright_deg=7.3 *degtorad
double precision, parameter::xi1_taperright_deg=7.7*degtorad

double precision, parameter::eta0_taperleft_deg=0.3*degtorad  
double precision, parameter::eta1_taperleft_deg=0.7*degtorad
double precision, parameter::eta0_taperright_deg=10.3*degtorad 
double precision, parameter::eta1_taperright_deg=10.7*degtorad


!Similarly, I just "know" where the slab possiblly exist.
double precision, parameter::dlat_searchrange=2.0
double precision, parameter::dlon_searchrange=2.0
!The maximum vertical thickness of slab. It equals thickness_slab*cos(dip)
!We use it to narrower the range of slab searching. Because some points are 
!too deep to be within slab.
double precision, parameter::max_vertical_thickness_slab=300000.0
integer ::nlat_searchrange,nlon_searchrange
integer ::ilat0_search,ilat1_search,ilon0_search,ilon1_search

!we need to search the point projected on slab interface, which has the minimum
! distance to the target point. The way to find this point is searching it around 
! the point on slab interface with the same lat/lon. The search range are given
! here.
nlat_searchrange=int (dlat_searchrange/dlat_slabmodel)
nlon_searchrange=int (dlon_searchrange/dlon_slabmodel)
!print *,'nlat',nlat_searchrange,nlon_searchrange
...
!piecewise linear interpolation to find the depth of slab top interface at this GLL point
  depth_slabtop=(slabdepth(ilon_find,ilat_find,1)*(1.0-ilon_realtype)+&
                 slabdepth(ilon_find+1,ilat_find,1)*ilon_realtype)*(1.0-ilat_realtype)+ &
                (slabdepth(ilon_find,ilat_find+1,1)*(1.0-ilon_realtype)+&
                 slabdepth(ilon_find+1,ilat_find+1,1)*ilon_realtype)*ilat_realtype
  depth_slabbot=(slabdepth(ilon_find,ilat_find,nlayers_slab)*(1.0-ilon_realtype)+&
                 slabdepth(ilon_find+1,ilat_find,nlayers_slab)*ilon_realtype)*(1.0-ilat_realtype)+ &
                (slabdepth(ilon_find,ilat_find+1,nlayers_slab)*(1.0-ilon_realtype)+&
                 slabdepth(ilon_find+1,ilat_find+1,nlayers_slab)*ilon_realtype)*ilat_realtype
...
!above slab or slab doesn't exist at all.
  if(depth<depth_slabtop.or.depth>depth_slabbot.or.depth_slabtop<-1.e4) then
    ...
    do while(ilayer_renewed.le.nlayers_slab.and.depth.gt.depth_renewedlayer)
        depth_oldlayer=depth_renewedlayer
        ilayer_renewed=ilayer_renewed+1
        depth_renewedlayer=(slabdepth(ilon_find,ilat_find,ilayer_renewed)*(1.0-ilon_realtype)+&
                 slabdepth(ilon_find+1,ilat_find,ilayer_renewed)*ilon_realtype)*(1.0-ilat_realtype)+&
                (slabdepth(ilon_find,ilat_find+1,ilayer_renewed)*(1.0-ilon_realtype)+&
                 slabdepth(ilon_find+1,ilat_find+1,ilayer_renewed)*ilon_realtype)*ilat_realtype
    end do
    ratio_between_depths=(depth-depth_oldlayer)/(depth_renewedlayer-depth_oldlayer)
    taper_renewed=(slabtaper(ilon_find,ilat_find,ilayer_renewed)*(1.0-ilon_realtype)+&
                 slabtaper(ilon_find+1,ilat_find,ilayer_renewed)*ilon_realtype)*(1.0-ilat_realtype)+&
                (slabtaper(ilon_find,ilat_find+1,ilayer_renewed)*(1.0-ilon_realtype)+&
                 slabtaper(ilon_find+1,ilat_find+1,ilayer_renewed)*ilon_realtype)*ilat_realtype
    dep_slabcoord_renewed=thickness_onelayer*(ilayer_renewed-1)

    taper_old=(slabtaper(ilon_find,ilat_find,ilayer_renewed-1)*(1.0-ilon_realtype)+&
                 slabtaper(ilon_find+1,ilat_find,ilayer_renewed-1)*ilon_realtype)*(1.0-ilat_realtype)+&
                (slabtaper(ilon_find,ilat_find+1,ilayer_renewed-1)*(1.0-ilon_realtype)+&
                 slabtaper(ilon_find+1,ilat_find+1,ilayer_renewed-1)*ilon_realtype)*ilat_realtype
    dep_slabcoord_old=thickness_onelayer*(ilayer_renewed-2)

!piecewise linear interpolation
    taper_thispoint=taper_renewedlayer*ratio_between_depths+taper_oldlayer*(1.0-ratio_between_depths)
...
!eta left taper
if(y_cubedsph.lt.eta0_taperleft_deg) then
   perturb=0.0
else if(y_cubedsph.gt.eta0_taperleft_deg.and.y_cubedsph.lt.eta1_taperleft_deg) then
   perturb=perturb*(1.0+(y_cubedsph-eta1_taperleft_deg)/(eta1_taperleft_deg-eta0_taperleft_deg))
end if


!eta right taper
if(y_cubedsph.gt.eta1_taperright_deg) then
   perturb=0.0
else if(y_cubedsph.gt.eta0_taperright_deg.and.y_cubedsph.lt.eta1_taperright_deg) then
   perturb=perturb*(1.0-(y_cubedsph-eta0_taperright_deg)/(eta1_taperright_deg-eta0_taperright_deg))
end if
