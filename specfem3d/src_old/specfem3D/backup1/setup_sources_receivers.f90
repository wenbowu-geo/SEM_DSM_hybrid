...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
      ...
      allocate(lat_source(NSOURCES))
      allocate(long_source(NSOURCES))
  end if
!WENBO_E
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
     call synchronize_all()
     if(DEBUG_COUPLING) print *,'locate_source',myrank,COUPLING_TYPE
      call locate_source_cubed(ibool,NSOURCES,myrank,NSPEC_AB,NGLOB_AB,xstore,ystore,zstore,&
      ...
      if(DEBUG_COUPLING) print *,'Mxx',Mxx,Myy,Mzz,Mxy,Mxz,Myz
   else
...
subroutine setup_receivers_cubed()
...
!     get total number of stations
    ...
    enddo
    close(IIN)
    if(DEBUG_COUPLING) print *,' NREC2',nrec
    if(nrec < 1) call exit_MPI(myrank,'need at least one receiver ')
    call synchronize_all()
...
!     get total number of stations
   ...
   do while(ios == 0)
    ...
    enddo
    close(IIN)
    if(DEBUG_COUPLING) print *,' NREC1',nrec
    if (nrec < 1) call exit_MPI(myrank, 'adjoint simulation needs at least one receiver')
    call synchronize_all()
  endif
...
! locate receivers in the mesh
  call locate_receivers_cubed(ibool,myrank,NSPEC_AB,NGLOB_AB,NGNOD, &
            xstore,ystore,zstore,xigll,yigll,zigll,rec_filename, &
            nrec,islice_selected_rec,ispec_selected_rec, &
            xi_receiver,eta_receiver,gamma_receiver,station_name,network_name,nu,&
            NPROC,theta_source(1),phi_source(1), CUBED_SPHERE_PROJECTION, &
            iglob_is_surface_external_mesh,ispec_is_surface_external_mesh, &
            num_free_surface_faces,free_surface_ispec,free_surface_ijk)
! count number of receivers located in this slice
  nrec_local = 0
  ...
  call synchronize_all()
! checks if acoustic receiver is exactly on the free surface because pressure is
! zero there
  call setup_receivers_check_acoustic()

end subroutine setup_receivers_cubed
!WENBO_E
