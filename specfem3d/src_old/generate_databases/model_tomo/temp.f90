...
!  enddo
  ...
  integer :: myrank
  integer ::ilat,ilon
  filename = MF_IN_DATA_FILES(1:len_trim(MF_IN_DATA_FILES))//'real_bathymetry_topography'
  open(unit=27,file=trim(filename),status='old',action='read',iostat=ier)
  if( ier /= 0 ) call exit_MPI(myrank,'error reading tomography file')
  ...
  read(27,*) NLON, NLAT
  allocate(toography(NLON,NLAT)
  do ilon=1,NLON
     do ilat=1,NLAT
        read(27,*)topography(ilat,ilon)
     end do
  end do
...
!South America
  if(izone2_Radi.eq.0.and.izone1_Radi.eq.0.and.abs(Radi-6367122.0).lt.10000.0) then
     if(fluid_thispoint.eq.1) then
        izone2_Radi=11
     else
        izone1_Radi=10
...
!            sin(phi_surf)=ystore(iglob)/r_surf/dsin(theta_surf)
              ...
              ilat=(theta_surf-ORIG_LAT)/SPACING_LAT
              ilon=(phi_surf-ORIG_LON)/SPACING_LON
              topo=topography(ilat,ilon)
              if(Radi-R_EARTH_SURF-topo.gt.-3000.0.and.&
                 Radi-R_EARTH_SURF-topo.lt.0.and.fluid_thispoint.eq.0) then
                   vs_final=2.0
                   vp_final=1.732*vs_final
                   rho_final=2.0
              end if
