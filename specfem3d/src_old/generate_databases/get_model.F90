...
!WENBO_S
          ...
          if(PML_CONDITIONS.and.CUBED_SPHERE_PROJECTION) then
            ...
            ymesh_cubedsph=0.0
            zmesh_cubedsph=0.0
            if(myrank.eq.0.and.ispec.eq.1.and.i.eq.1.and.j.eq.1.and.k.eq.1) &
            write(*,*) 'WARNING: the cubed sphere coordinates in generate &
                database are only computed for PML_CONDITIONS mode. Therefore,&
                the slab or tomography model is not tapered on the edges of SEM box!'
          end if
