...
!  integer :: icornerlat,icornerlong
  integer :: icornerxi,icornereta
  double precision :: lat,long
!  double precision :: long_corner,lat_corner,ratio_xi,ratio_eta
  double precision xi_corner,eta_corner,ratio_xi,ratio_eta
!WENBO_E
...
!WENBO_S
  ! initializes
   if(CUBED_SPHERE_PROJECTION) then
     ...
     orig_x_interface_bottom = 0
     orig_y_interface_bottom = 0
     spacing_x_interface_bottom = ANGULAR_WIDTH_XI_IN_DEGREES
     spacing_y_interface_bottom = ANGULAR_WIDTH_ETA_IN_DEGREES
   else
...
!WENBO_S
  ...
  ! user output
  if (myrank == 0) then
   if(.not.CUBED_SPHERE_PROJECTION)
    write(IMAIN,*) 'mesh:'
...
!WENBO_E
  ...
  do ilayer = 1,number_of_layers
    ...
    if (myrank == 0) then
!WENBO_S
      if (.not.CUBED_SPHERE_PROJECTION.and.SUPPRESS_UTM_PROJECTION_TOP) then
        str_unit = str_unit_m
      else
...
!        y_current = UTM_Y_MIN + (dble(iproc_eta_current)+etan)*(UTM_Y_MAX-UTM_Y_MIN)/dble(NPROC_ETA)
        if(CUBED_SPHERE_PROJECTION) then
           x_current=(dble(iproc_xi__current)+xin)*(ANGULAR_WIDTH_XI_IN_DEGREES)/dble(NPROC_XI)
           y_current=(dble(iproc_eta_current)+etan)*(ANGULAR_WIDTH_ETA_IN_DEGREES)/dble(NPROC_ETA)
        else
           x_current = UTM_X_MIN + (dble(iproc_xi_current)+xin)*(UTM_X_MAX-UTM_X_MIN)/dble(NPROC_XI)
           y_current = UTM_Y_MIN + (dble(iproc_eta_current)+etan)*(UTM_Y_MAX-UTM_Y_MIN)/dble(NPROC_ETA)
        end if
        ...
        if(CUBED_SPHERE_PROJECTION) then
           icornerxi = int((x_current - orig_x_interface_bottom) /spacing_x_interface_bottom) + 1
           icornereta = int((y_current - orig_y_interface_bottom) /spacing_y_interface_bottom) + 1
        else
           ...
           icornerxi = int((lat - orig_y_interface_bottom) / spacing_y_interface_bottom) + 1
           icornereta = int((long - orig_x_interface_bottom) / spacing_x_interface_bottom) + 1
        end if
...
! avoid edge effects and extend with identical point if outside model
         if(icornerxi < 1) icornerxi = 1
         if(icornerxi > npx_interface_top-1) icornerxi = npx_interface_top-1
         if(icornereta < 1) icornereta = 1
         if(icornereta > npy_interface_top-1) icornereta = npy_interface_top-1
...
!        ratio_eta = (lat - lat_corner) / spacing_y_interface_bottom
         ...
         xi_corner=orig_x_interface_bottom + (icornerxi-1)*spacing_x_interface_bottom
         eta_corner=orig_y_interface_bottom + (icornereta-1)*spacing_y_interface_bottom
         if(CUBED_SPHERE_PROJECTION) then
             ratio_xi = (x_current - xi_corner) / spacing_x_interface_bottom
             ratio_eta = (y_current - eta_corner) / spacing_y_interface_bottom
         else
! compute ratio for interpolation
!                ratio_xi = (long - long_corner) / spacing_x_interface_bottom
!                ratio_eta = (lat - lat_corner) / spacing_y_interface_bottom
             ratio_xi = (long - xi_corner) / spacing_x_interface_bottom
             ratio_eta = (lat - eta_corner) / spacing_y_interface_bottom
         end if
!WENBO_E
...
! interpolate elevation at current point
          z_interface_bottom = &
               interface_bottom(icornerxi,icornereta)*(1.-ratio_xi)*(1.-ratio_eta) + &
               interface_bottom(icornerxi+1,icornereta)*ratio_xi*(1.-ratio_eta) + &
               interface_bottom(icornerxi+1,icornereta+1)*ratio_xi*ratio_eta + &
               interface_bottom(icornerxi,icornereta+1)*(1.-ratio_xi)*ratio_eta
!WENBO_E
...
! get top interface value
        if(CUBED_SPHERE_PROJECTION) then
           icornerxi=int((x_current - orig_x_interface_top) / spacing_x_interface_top) + 1
           icornereta = int((y_current - orig_y_interface_top) / spacing_y_interface_top) + 1
        else
! project x and y in UTM back to long/lat since topo file is in long/lat
           call utm_geo(long,lat,x_current,y_current,UTM_PROJECTION_ZONE,IUTM2LONGLAT,SUPPRESS_UTM_PROJECTION_TOP)

! get coordinate of corner in bathy/topo model
           icornerxi = int((long - orig_x_interface_top) / spacing_x_interface_top) + 1
           icornereta = int((lat - orig_y_interface_top) / spacing_y_interface_top) + 1
        end if
!WENBO_E
...
!WENBO_S        ! avoid edge effects and extend with identical point if outside model
!        if (icornerlong < 1) icornerlong = 1
...
!        lat_corner = orig_y_interface_top + (icornerlat-1)*spacing_y_interface_top
         xi_corner = orig_x_interface_top + (icornerxi-1)*spacing_x_interface_top
         eta_corner = orig_y_interface_top + (icornereta-1)*spacing_y_interface_top
!WENBO_E
...
!         ratio_eta = (lat - lat_corner) / spacing_y_interface_top
         ratio_xi = (long - xi_corner) / spacing_x_interface_top
         ratio_eta = (lat - eta_corner) / spacing_y_interface_top
        end if
!WENBO_E
...
! interpolate elevation at current point
        z_interface_top = &
             interface_top(icornerxi,icornereta)*(1.-ratio_xi)*(1.-ratio_eta) + &
             interface_top(icornerxi+1,icornereta)*ratio_xi*(1.-ratio_eta) + &
             interface_top(icornerxi+1,icornereta+1)*ratio_xi*ratio_eta + &
             interface_top(icornerxi,icornereta+1)*(1.-ratio_xi)*ratio_eta
!WENBO_E
