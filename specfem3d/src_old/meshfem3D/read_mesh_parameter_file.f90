...
!WENBO_S
!add the paramter COUPLING_TYPE
  use meshfem3D_par, only: LATITUDE_MIN,LATITUDE_MAX,LONGITUDE_MIN,LONGITUDE_MAX, &
    ...
    myrank,sizeprocs,COUPLING_TYPE
!WENBO_E
...
!WENBO_S
  use SEMtoTele_MeshPar, only: ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES
!WENBO_E
...
!WENBO_S
  double precision, parameter ::xi_min=0.0
  double precision, parameter ::eta_min=0.0
!WENBO_E
...
!WENBO_S
  call read_value_logical_mesh(IIN,IGNORE_JUNK,CUBED_SPHERE_PROJECTION,'mesher.CUBED_SPHERE_PROJECTION_PROJECTION',ier)
  if (ier /= 0) stop 'Error reading Mesh parameter CUBED_SPHERE_PROJECTION_PROJECTION'
!WENBO_E
...
!WENBO_S
!For the DSM-SEM coupling mode, LONGITUDE_MIN,LATITUDE_MIN, LONGITUDE_MAX and LATITUDE_MAX are not 
!functional. Instead, we use ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES to specify 
!the size dimension. xi_min and eta_min are constant 0.0.
  if(COUPLING_TYPE.ne.4) then
    call utm_geo(xi_min,eta_min,UTM_X_MIN,UTM_Y_MIN,UTM_PROJECTION_ZONE,ILONGLAT2UTM,SUPPRESS_UTM_PROJECTION)
    call utm_geo(ANGULAR_WIDTH_XI_IN_DEGREES,ANGULAR_WIDTH_ETA_IN_DEGREES,UTM_X_MAX,UTM_Y_MAX, &
