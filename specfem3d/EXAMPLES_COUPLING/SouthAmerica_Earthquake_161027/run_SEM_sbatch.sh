...
echo "running SouthAmerica_Earthquake_161027 example: `date`"
currentdir=`pwd`
mpiruncmd="mpirun"
...
#(free surface, upper crust interface at 15 km and Moho at 35 km). You need to list the 
#topography files corresponding to each of them listed in the file DATA/meshfem3D_files/interfaces.dat. 
...
#The second parameter "./DATA/meshfem3D_files/SA_real_topo.dat" shows the name of topography 
...
jobid_mesh_tmp=$(sbatch run_meshfem3D.pbs)
jobid_mesh=`echo ${jobid_mesh_tmp} | cut -d ' ' -f 4 | tr -d ' '`
echo jobid_mesh $jobid_mesh
...
#submit a generate_database job and launch it after meshing is finished (using job dependency option).
jobid_gen_tmp=$(sbatch --dependency=afterok:$jobid_mesh run_generate_databases.pbs)
echo $jobid_mesh
...
jobid_spec=`sbatch --dependency=afterok:${jobid_gen} run_specfem3d.pbs`
echo "The job of solver submited, running this job takes about 9 hours..."
...
echo 
echo
echo "Waiting for jobs Done! Next, move to the DSM step and run DSM for Green's Function."
echo `date`
