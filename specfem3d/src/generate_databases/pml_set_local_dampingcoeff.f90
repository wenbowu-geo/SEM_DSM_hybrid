...
!WENBO_S
  use generate_databases_par, only: ibool,NGLOB_AB,d_store_x,d_store_y,d_store_z, &
                                    ...
                                    ZERO,ONE,TWO,HUGEVAL,COUPLING_TYPE
...
!WENBO_E
  ...
  real(kind=CUSTOM_REAL) :: pml_damping_profile_l,vp
!WENBO_S
! Round-off error happens if dist defined as double precision type. It happens
! to both hybrid method and original code. Thus,
! We defined it as real type to get rid of round-off error (drop the 8(9 or 7??) decimals)).
!Maybe there is better way to deal with it.
!  real(kind=4) :: dist
...
!WENBO_S
  if(DEBUG_COUPLING) then
     print *,'check_CPML',CPML_width_x_right,x_max_all,CPML_x_right,&
                x_min_all,CPML_x_left,nspec_cpml,myrank,nspec_cpml
  end if
!WENBO_E
...
!WENBO
    write(IMAIN,*) 'CPML_width_xRL: ',CPML_width_x_left_max_all,CPML_width_x_right_max_all
...
!WENBO_S
              if(COUPLING_TYPE.eq.1) then
                 d_x = pml_damping_profile_l(myrank,iglob,dist,vp,CPML_width_x*zstore(iglob))
              else 
...
!WENBO_S
             if(COUPLING_TYPE.eq.1) then
                 d_y = pml_damping_profile_l(myrank,iglob,dist,vp,CPML_width_y*zstore(iglob))
             else
...
!WENBO_S
  if (myrank == 0.and.DEBUG_COUPLING) print *, 'min_distance_between_CPML_parameter', min_distance_between_CPML_parameter
!WENBO_E
! for robust parameter separation of PML damping parameter
...
!WENBO
              print *,'beta',beta_z,alpha_x,alpha_y
...
function pml_damping_profile_l(myrank,iglob,dist,vp,delta)
  ...
  use constants, only:     NDECIMAL_REMAINED

  implicit none

  integer, intent(in) :: myrank,iglob

  real(kind=CUSTOM_REAL), intent(in) :: vp,delta
!WENBO
!  real(kind=CUSTOM_REAL), intent(in) :: dist
  real(kind=CUSTOM_REAL) :: dist
!  real(kind=4), intent(in) :: dist

  real(kind=CUSTOM_REAL) :: pml_damping_profile_l
  real(kind=CUSTOM_REAL) :: dist_truncation
  

!WENBO_S
  dist_truncation=int(dist*10.0**(NDECIMAL_REMAINED))*10.0**(-NDECIMAL_REMAINED)
  dist=dist_truncation
!WENBO_E
...
!WENBO_S
    if (CUSTOM_REAL == SIZE_REAL) then
      ...
      !pml_damping_profile_l = - sngl(((NPOWER + 1.d0) * dble(vp) * log(CPML_Rcoef) / &
      !                                    (TWO * dble(delta))) * dble(dist_truncation)**(1.2d0 * NPOWER))
    else
      pml_damping_profile_l = - ((NPOWER + 1.d0) * vp * log(CPML_Rcoef) / (TWO * delta)) * dist**(1.2d0 * NPOWER)
      !pml_damping_profile_l = - ((NPOWER + 1.d0) * vp * log(CPML_Rcoef) / (TWO * delta)) * dist_truncation**(1.2d0 * NPOWER)
!WENBO_E
    endif
  else
