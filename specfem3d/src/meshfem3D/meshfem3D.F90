...
!WENBO_S
!For DSM-SEM coupling, COUPLING_TYPE=1,2 or 3
...
!WENBO_S
  !NER,iproc_xi_current and iproc_eta_current are used below.
  !So do not move this section before!
  if(COUPLING_TYPE.lt.4) then
  ! not delete it, make sure everybody is synchronized
   call synchronize_all()
   call prepare_variables_SEMtoTele(myrank)
  end if
!WENBO_E
