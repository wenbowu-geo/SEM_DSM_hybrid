...
subroutine save_databases_adios(LOCAL_PATH, myrank, sizeprocs, &
                                ...
                                nodes_coords_cubedsph,CUBED_SPHERE_PROJECTION,&
...
!WENBO_S
  double precision :: nodes_coords_cubedsph(nglob,3)
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
     call define_adios_global_array1D(group, groupsize, local_dim, "", STRINGIFY_VAR(nodes_coords_cubedsph))
  end if
!WENBO_E
...
!WENBO_S
  if(CUBED_SPHERE_PROJECTION) then
  call write_adios_global_1d_array(handle, myrank, sizeprocs, local_dim, &
                         "nodes_coords_cubedsph", transpose(nodes_coords_cubedsph))
  end if
!WENBO_E
