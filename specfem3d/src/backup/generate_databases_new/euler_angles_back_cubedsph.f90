...
! compute the Euler angles and the associated rotation matrix
  ...
  subroutine euler_angles_back_cubedsph(rotation_matrix_back_cubedsph,&
...
!  include "constants.h"
  ...
  double precision rotation_matrix_back_cubedsph(3,3)
...
! compute colatitude and longitude and convert to radians
  alpha = CENTER_LONGITUDE_IN_DEGREES * degrad
  beta = (90.0d0 - CENTER_LATITUDE_IN_DEGREES) * degrad
  gamma = GAMMA_ROTATION_AZIMUTH * degrad
...
!For Euler rotation matrix, inverse(rotation_matrix)=transpose(rotation_matrix)
  rotation_matrix_back_cubedsph=transpose(rotation_matrix)
