...
!local parameters for reading file
  ...
  DSM_file_name = IN_DATA_FILES_PATH(1:len_trim(IN_DATA_FILES_PATH))//&
                  trim(DSM_FILENAME)
...
!solid media
  if(flag_media.eq.-1) then
      fluid_thispoint=0
!fluid media
  else if(flag_media.eq.-2) then
      fluid_thispoint=1
  else
      stop 'fluid_thispoint shoud be either -1 or -2.'
  end if
  ...
  do izone=n_structure_zone,1,-1
     if(dabs(rmax_structure_zone(izone)-Radi).le.depth_tolerence) then
        izone1_Radi=izone
...
!Bathymetry
!Gofar fault
!  if(izone2_Radi.eq.0.and.izone1_Radi.eq.0.and.dabs(Radi-6368000.0).lt.1900.0) then
...
!South America
  if(izone2_Radi.eq.0.and.izone1_Radi.eq.0.and.dabs(Radi-6367122.0).lt.10000.0) then
     if(fluid_thispoint.eq.1) then
        izone2_Radi=11
     else
        izone1_Radi=10
...
!for deep coupling check
!  if(dabs(x_eval-6248350.5253).lt.10.0.and.dabs(y_eval-1114407.23).lt.10.0&
!     .and.dabs(z_eval+24924.49).lt.10.0) &
!    print *,'found_result',Radi,izone1_Radi,izone2_Radi,rho_final,vp_final,vs_final
  
!  if(myrank.eq.1) print *,'structure',6371000.0-Radi,vp_final,vs_final,rho_final, &
!          izone1_Radi,izone2_Radi,rmin_structure_zone(izone1_Radi)
...
!         print *,'ixiyiz',ix,iy,iz
  ...
  if(z_tomography(p4+1) == z_tomography(p0+1)) then
     ...
     if(dabs(z_tomography(p4+1)-z_tomography(p0+1)).lt.1.e-5) &
  ...
  if(z_tomography(p5+1) == z_tomography(p1+1)) then
     ...
     if(dabs(z_tomography(p5+1)-z_tomography(p1+1)).lt.1.e-5) &
  ...
  if(z_tomography(p6+1) == z_tomography(p2+1)) then
     ...
     if(dabs(z_tomography(p6+1)-z_tomography(p2+1)).lt.1.e-5) &
  ...
  if(z_tomography(p7+1) == z_tomography(p3+1)) then
     ...
     if(dabs(z_tomography(p7+1)-z_tomography(p3+1)).lt.1.e-5) &
...
!WENBO
     ...
     if(vs_final.gt.900.0) then
       vp_final=vp_final*(1+2.0*perturbation)
